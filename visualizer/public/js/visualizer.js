    ...
    drawFrame: function (b) {
        ...
        for (var g = 0; g < c.length; g++) {
            ...
            if (k.x && k.y)
            {
	            ...
	            n.fillStyle = this.config.teamColor[k.owner];
	            n.beginPath();
	            n.save();
...
(function (c) {
    ...
    c("#display").bind("drawn", function () {
        var e = Visualizer.frame;
        c("#turnCounter").html("Turn: " + Math.floor(Visualizer.frame + 1) + " of " + Visualizer.moves.length);
        c(".player1Name").text(Visualizer.players[0] + " (" + Visualizer.shipCount[1] + "/" + Visualizer.growthRate[1] + ")");
        c(".player2Name").text(Visualizer.players[1] + " (" + Visualizer.shipCount[2] + "/" + Visualizer.growthRate[2] + ")")
    });
